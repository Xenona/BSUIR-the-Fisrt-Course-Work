object FBigPic: TFBigPic
  Left = 0
  Top = 0
  ClientHeight = 870
  ClientWidth = 1207
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object LabelGenre: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 214
    Width = 88
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1046#1072#1085#1088':'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelTheme: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 270
    Width = 169
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1058#1077#1084#1072#1090#1080#1082#1072':'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelMaterials: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 334
    Width = 186
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1052#1072#1090#1077#1088#1080#1072#1083#1099':'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelDescr: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 398
    Width = 217
    Height = 88
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1050#1086#1088#1086#1090#1082#1086#1077' '#1086#1087#1080#1089#1072#1085#1080#1077': '
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelUserRate: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 652
    Width = 266
    Height = 72
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1100#1089#1082#1072#1103' '#1086#1094#1077#1085#1082#1072': '
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelUserComm: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 737
    Width = 290
    Height = 91
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1100#1089#1082#1080#1081' '#1082#1086#1084#1084#1077#1085#1090#1072#1088#1080#1081
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldTitle: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 21
    Width = 410
    Height = 107
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = 'Master Jan Hus Preaching at the Bethlehem Chapel'
    Color = 4227327
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 35
    Font.Name = 'Montserrat SemiBold'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldYoS: TLabel
    AlignWithMargins = True
    Left = 591
    Top = 98
    Width = 88
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = '1910'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 25
    Font.Name = 'Montserrat'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldYoE: TLabel
    AlignWithMargins = True
    Left = 679
    Top = 98
    Width = 88
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = '1920'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 25
    Font.Name = 'Montserrat'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldYoW: TLabel
    AlignWithMargins = True
    Left = 767
    Top = 98
    Width = 37
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = '10'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 25
    Font.Name = 'Montserrat'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldGenre: TLabel
    AlignWithMargins = True
    Left = 907
    Top = 214
    Width = 120
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1052#1086#1076#1077#1088#1085
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 30
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldTheme: TLabel
    AlignWithMargins = True
    Left = 907
    Top = 270
    Width = 169
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1051#1102#1076#1080
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 30
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldMats: TLabel
    AlignWithMargins = True
    Left = 907
    Top = 334
    Width = 186
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1061#1086#1083#1089#1090', '#1084#1072#1089#1083#1086
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 30
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldDescr: TLabel
    AlignWithMargins = True
    Left = 907
    Top = 390
    Width = 295
    Height = 88
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1044#1072', '#1086#1095#1077#1085#1100' '#1080#1085#1090#1077#1088#1077#1089#1085#1086', '#1083#1102#1073#1083#1102' '#1082#1091#1088#1089#1072#1095
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 30
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelFieldUsrCmm: TLabel
    AlignWithMargins = True
    Left = 907
    Top = 741
    Width = 286
    Height = 156
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1100#1089#1082#1080#1081' '#1082#1086#1084#1084#1077#1085#1090#1072#1088#1080#1081
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 30
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object LabelDash: TLabel
    AlignWithMargins = True
    Left = 647
    Top = 98
    Width = 41
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = #8212
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 25
    Font.Name = 'Montserrat'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object Label1: TLabel
    AlignWithMargins = True
    Left = 743
    Top = 98
    Width = 41
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = '('
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 25
    Font.Name = 'Montserrat'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object Label2: TLabel
    AlignWithMargins = True
    Left = 804
    Top = 98
    Width = 41
    Height = 35
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    AutoSize = False
    Caption = ')'
    Color = clBlack
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 25
    Font.Name = 'Montserrat'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
    WordWrap = True
  end
  object PanelPic: TPanel
    Left = 0
    Top = 0
    Width = 570
    Height = 870
    Align = alLeft
    Caption = 'Panel1'
    TabOrder = 0
    ExplicitLeft = -8
    ExplicitTop = 40
    object ImageItself: TImage
      Left = 0
      Top = 98
      Width = 525
      Height = 525
    end
  end
  object ChkBxFavourite: TCheckBox
    Left = 1153
    Top = 29
    Width = 18
    Height = 33
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
  end
  object CmbBxUserRate: TComboBox
    Left = 907
    Top = 655
    Width = 238
    Height = 35
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -20
    Font.Name = 'Montserrat'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
  end
end
