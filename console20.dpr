...
function searchDataInit(head: PPicElem; infoToSearch: string; fieldWhereSearch: integer ): PPicElem;
var
  curElem: PPicElem;
  newHead: PPicElem;
  searchedList: PPicElem;
begin
  curElem := head;

  new(searchedList);
  newHead := searchedList;


  while curElem^.Next <> nil do
  begin
    if isRecordMatch(curElem^.data, infoToSearch, fieldWhereSearch) then
    begin

      searchedList.data := curElem.data;
      new(searchedList^.Next);
...
function searchData(head: PPicElem; infoToSearch: string; fieldWhereSearch: integer ): PPicElem;
var
  curElem: PPicElem;
  newList: PPicElem;
